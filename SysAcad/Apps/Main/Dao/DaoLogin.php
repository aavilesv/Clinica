...
class DaoUsuarioModel extends DBAAbstractModel implements IntUsuario
{
  ...
  public function consultarModulos($variable, $opcion) {
        $modulos = array();
        try {
            if ($opcion == 1) {
                $sql = "Select idModulo,modNombre,modDescripcion,codigo,modFoto,modPadre, color
                    From segmodulo Where modPadre= ?";
            } else {
                $sql = "Select idModulo,modNombre,modDescripcion,codigo,modFoto,modPadre,color
