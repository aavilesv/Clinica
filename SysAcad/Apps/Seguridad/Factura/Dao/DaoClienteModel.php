...
/*
    ...
    public function consultarPacientes()
    {
      $Clientes = array();
        try {
            $sql = "Select PAC_Id, PAC_Nombre, PAC_Cedula,PAC_Apellido From paciente
			      Order By PAC_Apellido";
            $stmt = $this->getConexion()->prepare($sql);
            $stmt->execute();
            $rs = $stmt->get_result();
            while (($Client = $rs->fetch_object())) {
              $Clientes[] = array('id' => $Client->PAC_Id,'name' => $Client->PAC_Nombre." ".$Client->PAC_Apellido);
            }
            $rs->close();
        } catch (Exception $ex) {
            throw $ex->getMessage();
    ...
    public function consultarPaciente($id)
    {
      ...
      $Clientes;
        try {
            $sql = "Select PAC_Nombre, PAC_Cedula,PAC_Apellido,PAC_Ciudad From paciente
			      where PAC_Id = ?";
            $stmt = $this->getConexion()->prepare($sql); //Prepara una sentencia para su ejecución y devuelve un objeto sentencia
            $stmt->bind_param('i', $id); //Agrega variables a una sentencia preparada como parámetros
            $stmt->execute();
            $rs = $stmt->get_result();
            if ($tmp = $rs->fetch_object()) {
              $Clientes = array('name'=>$tmp->PAC_Nombre." ".$tmp->PAC_Apellido,'ced' => $tmp->PAC_Cedula, 'dire' => $tmp->PAC_Ciudad);
            }
            $rs->close();
        } catch (Exception $ex) {
            throw $ex->getMessage();
